...
async function getContatoPadrao(){
  ...
  const novo=await bling(`/contatos`,{method:"POST",body:JSON.stringify({nome:"CONSUMIDOR FINAL", tipo:"F"})});
  ...
  try{
    ...
    tab.model.forEach(c=>{
      ...
      (c.itens||[]).forEach(it=>{
        ...
        cats[c.t].produtos.push({id:it.id,nome:it.nome,obs:it.obs||"",preco:it.preco,un:it.caixa||1,sabores,estoqueTotal,imagem});
      });
    });
